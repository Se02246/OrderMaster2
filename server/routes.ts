...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ... (tutto il codice di registrazione, login, logout, /me rimane invariato) ...
  ...
  router.use(isAuthenticated); // Questo è corretto

  // === INIZIO NUOVA ROTTA PER IL TEMA ===
  router.put("/users/theme", async (req: Request, res: Response) => {
    try {
      const userId = getUserId(req);
      
      // Validazione del colore in arrivo
      const validationSchema = z.object({
        color: z.string().max(50), // Es. "210 40% 98%"
      });
      const validationResult = validationSchema.safeParse(req.body);

      if (!validationResult.success) {
        const errorMessage = fromZodError(validationResult.error).message;
        return res.status(400).json({ message: errorMessage || "Colore non valido." });
      }

      const { color } = validationResult.data;

      // Aggiorna il colore nel database per l'utente loggato
      const [updatedUser] = await db
        .update(users)
        .set({ theme_color: color })
        .where(eq(users.id, userId))
        .returning(); // Ritorna i dati aggiornati dell'utente

      if (!updatedUser) {
        return res.status(404).json({ message: "Utente non trovato" });
      }

      // Rimuovi la password e invia i dati aggiornati al client
  ...
  // Statistics endpoint
  // ... (la rotta /statistics rimane invariata) ...
